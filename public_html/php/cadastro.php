...
$nmLogin = htmlspecialchars_decode($_POST["nmLogin"]) or volta(1);
$nmSenha = $_POST["nmSenha"] or volta(1);
$nmSenha = crypt($nmSenha, salt());
$nmEmail = htmlspecialchars_decode($_POST["nmEmail"]) or volta(1);
$cdCidade = htmlspecialchars_decode($_POST["cdCidade"]) or volta(1);
$rbTipo = $_POST["rbTipo"] or volta(1);

# Cadastrar usuário e o perfil relacionado a ele
if($rbTipo == "A") { # A = aluno
	require_once("../../aluno.class.php");
	$nmAluno = htmlspecialchars_decode($_POST["nmAluno"]) or volta(1);
	$dtNascimento = $_POST["dtNascimento"] or volta(1);
	$usuario = new Usuario($nmLogin, $nmSenha);
	...
	$nmEscola = htmlspecialchars_decode($_POST["nmEscola"]) or volta(1);
